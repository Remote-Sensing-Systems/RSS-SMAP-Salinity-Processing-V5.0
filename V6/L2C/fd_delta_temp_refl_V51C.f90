! TM 02/17/2023
! adapted to V5.1
! new table delta_refl_temp_V51C.dat that was derived from V5.0 analysis at high latitudes
! I now use slighlty different TREFL for V-pol and h-pol

! TM 03/22/2018
! adjustment ot reflector temperature for SMAP V3

subroutine fd_delta_temp_refl_V51C(secyr, xang,  delta_temp_refl)  
...
! in V3 I do a time wrap at both ends
...
if (istart==1) then
    ...
    open(unit=3,file=temp_tab_file,form='binary',action='read',status='old')
...
return
end subroutine fd_delta_temp_refl_V51C